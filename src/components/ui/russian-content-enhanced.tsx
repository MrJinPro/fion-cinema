...
export function RussianContentEnhanced() {
  ...
  const { data: categories, isLoading: isLoadingCategories } = useKinopoiskCategories();
  const { data: categoryMovies, isLoading: isLoadingCategory } = useCategoryMovies(activeCategory);
  const { data: russianMoviesDB, isLoading: isLoadingRussianDB } = useRussianMoviesFromDB();
  ...
  return (
    <div className="space-y-8">
      <Alert className="bg-info/10 border-info/20">
        ...
        <AlertDescription>
          Новый улучшенный каталог российского контента с парсингом актуальных данных с Кинопоиск.ru.
        ...
        {categories && (
          <div>
            ...
            {isLoadingCategory ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {categoryMovies.slice(0, 18).map((movie) => (
                  <div key={movie.id} className="group cursor-pointer" onClick={() => handleKinopoiskClick(parseInt(movie.id))}>
